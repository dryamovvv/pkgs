...
FAILED_PKGS=""
JOBS=$(gh api "repos/${REPO}/actions/runs/${RUN_ID}/jobs" --jq '.jobs[] | select(.conclusion == "failure") | .name' 2>/dev/null) || JOBS=""
...
if [ -z "$FAILED_PKGS" ]; then
	FAILED_PKGS=$(echo "$PACKAGES_JSON" | jq -r '.[]' 2>/dev/null | tr '\n' ' ') || FAILED_PKGS=""
fi

echo "DEBUG: FAILED_PKGS final=$FAILED_PKGS" >&2
